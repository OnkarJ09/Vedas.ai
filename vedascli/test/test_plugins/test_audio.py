from .test_translator import Test_Vedas
...
def test_say(text, language=None):
    """
...
def test_translate_to_english(text, language):
    """
    ...
    """
    if language != 'en':
        translator = Test_Vedas.translate_text(text, 'en')
...
def test_take_command(language='en'):
    ...
    with mic as source:
        test_say(f"Listening in {language}...", language)
    ...
    try:
        ...
        test_say(f"Recognized command: {query}", language)
        query = test_translate_to_english(query, language)

        if "change language" in query.lower():
            new_language = detect_language_from_command(query)
            if new_language:
                test_say(f"Changing language to {new_language}", new_language)
                return new_language, True
            else:
                test_say("Sorry, I couldn't recognize the new language.", language)
                return language, False
        else:
            return query, False

    except sr.UnknownValueError:
        test_say("Sorry, I could not understand the audio.", language)
        return None, False
    except sr.RequestError as e:
        test_say(f"Could not request results; {e}", language)
        return None, False


def test_detect_language_from_command(command):
    """
...
"""
This is for complete test run in loop
"""
